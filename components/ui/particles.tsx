...
export function Particles({
  ...
  const resolvedColor = color ? color : isDark ? "#ffffff" : "#0f0f17";
  ...
  const onMouseMove = () => {
    if (canvasRef.current) {
      ...
      const inside =
        mousePosition.current.x < rect.right &&
        mousePosition.current.x > rect.left &&
        mousePosition.current.y < rect.bottom &&
        mousePosition.current.y > rect.top;
      if (inside) {
        mouse.current.x = x;
        mouse.current.y = y;
      }
