...
export function SiteNav({ activeSection, onNavigate }: SiteNavProps) {
  ...
  return (
    <>
      <aside
        className={`hidden md:flex shrink-0 border-r border-black/20 dark:border-white/10 flex-col justify-center relative min-h-screen sticky top-0 h-screen bg-[#f8f8fa] dark:bg-[#0e0e11] z-30 transition-all duration-300 ease-in-out ${
          isOpen ? "w-52 sm:w-60 md:w-64 p-6 md:p-8" : "w-16 sm:w-20 p-3 items-center"
        }`}
      >
        ...
        {/* 4 indicator dots */}
        <div
          className={`absolute top-1/2 -translate-y-1/2 flex flex-col items-center space-y-3 z-20 transition-all duration-300 ${
            isOpen ? "right-0 translate-x-1/2" : "right-1/2 translate-x-1/2"
          }`}
        >
          {navItems.map((item) => {
            const isActive = activeSection === item.id;
